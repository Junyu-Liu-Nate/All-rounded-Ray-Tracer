...
struct RenderShapeData {
    ...
    std::shared_ptr<BVH> triangleBVH; // BVH tree for triangles if the primitive is a mesh
};
...
public:
    ...
    static void dfsParse(SceneNode* node, std::vector<RenderShapeData>& shapes, std::vector<SceneLightData>& lights, glm::mat4 ctm);
};
